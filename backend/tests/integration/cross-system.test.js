...
describe('Cross-System Integration Tests - External Service Dependencies', () => {
  ...
  describe('Twilio SMS Integration', () => {
    test('Should format SMS alert messages correctly', async () => {
      const burnSchedule = {
        farmId: 123,
        fieldId: 456,
        scheduledTime: new Date('2025-08-25T09:00:00'),
        originalTime: new Date('2025-08-25T10:00:00'),
        pm25Level: 45,
        safetyStatus: 'moderate'
      };
      
      const message = alertSystem.formatSMSMessage(burnSchedule);
      
      expect(message).toContain('BURN SCHEDULE');
      expect(message).toContain('Farm 123');
      expect(message).toContain('Field 456');
      expect(message).toContain('09:00');
      expect(message).toContain('PM2.5');
      expect(message.length).toBeLessThanOrEqual(160); // SMS character limit
    });

    test('Should validate phone numbers before sending', () => {
      const validNumbers = [
        '+12125551234',
        '+442079460958',
        '+861012345678'
      ...
      validNumbers.forEach(num => {
        expect(alertSystem.isValidPhoneNumber(num)).toBeTruthy();
      });
      
      invalidNumbers.forEach(num => {
        expect(alertSystem.isValidPhoneNumber(num)).toBeFalsy();
      });
    });

    test('Should batch SMS messages for efficiency', async () => {
      const recipients = [
        { farmId: 1, phone: '+12125551001' },
        { farmId: 2, phone: '+12125551002' },
        { farmId: 3, phone: '+12125551003' },
        { farmId: 4, phone: '+12125551004' },
        { farmId: 5, phone: '+12125551005' }
      ];
      
      const batches = alertSystem.createSMSBatches(recipients, 2); // Batch size 2
      
      expect(batches).toHaveLength(3); // 5 recipients / 2 per batch = 3 batches
    ...
    test('Should handle Twilio API errors gracefully', async () => {
      if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN) {
        console.warn('Skipping: Twilio credentials not configured');
        return;
      }
      
      // Try to send to invalid number
      const result = await alertSystem.sendSMS(
        'invalid-number',
        'Test message'
      );
