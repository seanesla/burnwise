...
router.post('/send', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!recipient_phone && !recipient_email) {
      throw new ValidationError('Either phone or email must be provided', 'recipient');
    }
    
    // Process the alert using AlertsAgent
    const alertData = {
      type,
      farm_id,
      burn_request_id,
    ...
    const io = req.app.get('io');
    const result = await alertsAgent.processAlert(alertData, null, io);
    ...
    logger.performance('manual_alert_send', duration, {
      alertId: result.alertId,
      deliveryMethod: recipient_phone ? 'sms' : 'email'
    });
    ...
    logger.error('Manual alert send failed', { error: error.message });
    throw error;
  }
}));
