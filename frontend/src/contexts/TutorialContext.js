...
export const TutorialProvider = ({ children }) => {
  ...
  const fetchTutorialData = async () => {
    try {
      ...
      try {
        const farmsResponse = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/farms`, { withCredentials: true });
      ...
      try {
        const weatherResponse = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/weather/current`, {
      ...
      try {
        const burnsResponse = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5001'}/api/burn-requests`, { withCredentials: true });
